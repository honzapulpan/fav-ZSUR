...
def kmeans(data,cl_nb, eps, mi):
    ...
    return data_classified, J[-1]


def kmeans_direct(data_tmp, cl_nb, eps):
    data_classified, J_last = kmeans(data_tmp, cl_nb, eps, data_tmp[np.random.choice(len(data_tmp), cl_nb, replace=False)])
    return data_classified, J_last
...
def kmeans_binary(data_tmp, cl_nb, eps):
    ...
    for i in range(cl_nb-1):
        ...
        data_class, J_last = kmeans(data_tmp, 2, eps, [data_tmp[max_ind[0],:],data_tmp[max_ind[1],:]])
        # beru větší třídu a na ní volám zase kmeans pro 2 třídy
        J_crit.append(J_last)
...
def get_crit_f(data):    
    ...
    for i in range(len(data)):
        mi = np.average(data[i],axis=0)
        dist_cl = dist_v(data[i],mi)
        J_crit += np.sum(dist_cl)
    return J_crit
